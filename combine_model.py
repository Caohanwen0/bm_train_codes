...
def get_tokenizer():
    tokenizer = Tokenizer.from_file('tokenizer/tokenizer_twitter_reddit_ccnews.json')
    return tokenizer
...
def combine_roberta():
    old_tokenizer = RobertaTokenizer.from_pretrained('nyu-mll/roberta-med-small-1M-1')
    new_tokenizer = get_tokenizer()
    model = RobertaForMaskedLM.from_pretrained('nyu-mll/roberta-med-small-1M-1')
    ...
    torch.save(model.state_dict(), '/data0/private/caohanwen/OpenSoCo/checkpoint/roberta-small/model.pt')
